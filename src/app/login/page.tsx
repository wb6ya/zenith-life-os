...
import { Loader2, Lock, Mail, ArrowRight, Github, Globe, AlertCircle } from "lucide-react";
...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res?.error) {
        // 🛑 في حال الخطأ فقط نوقف التحميل لنعرض الرسالة
        setError(lang === 'ar' ? "بيانات الدخول غير صحيحة" : "Invalid credentials");
        setLoading(false);
      } else {
        // ✅ في حال النجاح، لا نوقف التحميل! نتركه يدور حتى يتم النقل
        router.push("/");
        router.refresh();
      }
    } catch (err) {
  ...
  const handleSocialLogin = (provider: string) => {
      // هنا أيضاً يبدأ التحميل ولا يتوقف
      setLoading(true);
    ...
    <div className="min-h-screen bg-[#050505] flex items-center justify-center p-4 font-sans relative overflow-hidden" dir="ltr">
      ...
      <div className="w-full max-w-md bg-[#0F0F0F]/80 backdrop-blur-xl border border-white/10 p-8 rounded-[32px] shadow-2xl relative z-10 animate-in fade-in zoom-in-95 duration-700">
        ...
        <div className="text-center mb-8 animate-in slide-in-from-bottom-4 fade-in duration-700">
            ...
            <p className="text-gray-400 text-sm">{isAr ? "سجل دخولك للنظام" : "Login to System"}</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 animate-in slide-in-from-bottom-4 fade-in duration-700 delay-200 fill-mode-both" autoComplete="off">
            ...
            >
                {loading ? <Loader2 className="animate-spin" /> : <>{isAr ? "دخول" : "Login"} <ArrowRight size={18} className="group-hover:translate-x-1 transition-transform" /></>}
            </button>
        </form>

        <p className="text-center text-xs text-gray-500 mt-8 animate-in fade-in duration-1000 delay-300 fill-mode-both">
            {isAr ? "ليس لديك حساب؟" : "Don't have an account?"} <Link href="/register" className="text-blue-500 hover:underline font-bold decoration-blue-500 underline-offset-4 hover:text-blue-400 transition-colors">{isAr ? "إنشاء حساب جديد" : "Sign Up"}</Link>
        </p>
      </div>
    </div>
  );
