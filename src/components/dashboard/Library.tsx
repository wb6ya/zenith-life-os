...
export default function Library({ resources: initialResources }: { resources: any[] }) {
  ...
  const handleStartReading = async (id: string) => {
      ...
      const updated = resources.map(r => ({
          ...r,
          status: r._id === id ? 'reading' : (r.status === 'reading' ? 'pending' : r.status)
      }));
      setResources(updated);
      setSelectedBook(null);
      
      await startReadingBook(id); 
      toast.success(txt.toast_book_started || "Reading Started 📖");
  };
  ...
  const handleUpdateProgress = async (id: string, amount: number) => {
      ...
      const updated = resources.map(r => {
          if (r._id === id) {
              const newProgress = Math.min(r.completedUnits + amount, r.totalUnits);
              return { ...r, completedUnits: newProgress };
          }
          return r;
      });
      setResources(updated);

      await updateProgress(id, amount); 
  };

  const handleFinishBook = async (id: string) => {
      playSuccess();
      // ⚡ Optimistic Update
      const updated = resources.map(r => r._id === id ? { ...r, status: 'completed', completedUnits: r.totalUnits } : r);
      setResources(updated);
      
      await finishBook(id);
      toast.success(txt.toast_book_finished || "Book Completed! 🎉");
  };
  ...
  const handleFormSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
      ...
      if (res?.success) { 
          ...
          if (editingBook) {
              toast.success(txt.toast_book_updated || "Book Updated 🔄");
          } else {
              toast.success(txt.toast_book_added || "Book Added to Archive 📚");
          }
      }
  };
  ...
  const handleDelete = async () => {
      ...
      await deleteBook(id); 
      toast(txt.toast_book_deleted || "Book Removed 🗑️", {
  ...
  const Modals = () => {
      ...
      return createPortal(
          <AnimatePresence>
              {/* ✅ Added explicit keys to fix the error */}
              {isModalOpen && (
                  <motion.div 
                    key="modal-add-edit"
                    className="fixed inset-0 z-[200] bg-black/80 backdrop-blur-xl flex items-center justify-center p-4" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}>
                      <motion.div 
                      ...
                      >
                          ...
                          <form onSubmit={handleFormSubmit} className="flex-1 overflow-y-auto custom-scrollbar px-6 pb-6 space-y-5" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
                              <div className="relative group cursor-pointer w-full aspect-[21/9] rounded-2xl overflow-hidden border border-white/10 bg-[#151515] hover:border-purple-500/50 transition-all shadow-inner">
                                  ...
                                  {imageBase64 ? <img src={imageBase64} className="w-full h-full object-cover opacity-80 group-hover:opacity-100 transition-opacity" /> : <div className="w-full h-full flex flex-col items-center justify-center gap-2 text-gray-500 group-hover:text-purple-400"><ImageIcon size={24} /><span className="text-[10px] font-bold tracking-widest uppercase">{txt.lib_upload_cover || "UPLOAD COVER"}</span></div>}
                              </div>
              ...
              {selectedBook && (
                  <motion.div 
                    key="modal-details"
                    className="fixed inset-0 z-[200] bg-black/80 backdrop-blur-xl flex items-center justify-center p-4" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} onClick={() => setSelectedBook(null)}>
                      <motion.div 
                      ...
                      >
                          ...
                          <div className="w-full md:w-64 flex-shrink-0 flex flex-col gap-4">
                              ...
                              <div className="flex gap-2">
                                  <button onClick={() => { playClick(); setEditingBook(selectedBook); setImageBase64(selectedBook.image || ""); setIsModalOpen(true); }} className="flex-1 py-3 bg-white/5 hover:bg-white/10 text-white rounded-xl border border-white/5 transition-all flex items-center justify-center gap-2 text-xs font-bold"><Pencil size={14}/> {txt.lib_edit_entry || "EDIT"}</button>
                          ...
                          <div className="flex-1 flex flex-col relative z-10" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
                              ...
                              <div className="mt-auto">
                                  <button onClick={() => handleStartReading(selectedBook._id)} className="w-full py-4 bg-white text-black font-black rounded-2xl hover:bg-gray-200 transition-all text-sm flex items-center justify-center gap-2 shadow-[0_0_30px_rgba(255,255,255,0.1)] hover:scale-[1.01]">
              ...
              {isDeleteConfirmOpen && (
                  <motion.div 
                    key="modal-delete"
                    className="fixed inset-0 z-[250] bg-black/90 backdrop-blur-md flex items-center justify-center p-4" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}>
                      <motion.div initial={{scale:0.9}} animate={{scale:1}} className="bg-[#0A0A0A] border border-red-500/30 p-8 rounded-[32px] w-full max-w-sm text-center shadow-[0_0_50px_rgba(220,38,38,0.2)]" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
  ...
  return (
    <>
    <div className="bg-gradient-to-b from-[#0F0F0F] to-[#050505] rounded-[40px] border border-white/5 p-8 h-[750px] flex flex-col relative overflow-hidden group shadow-2xl" dir="ltr">
      ...
      <div className="flex justify-between items-center mb-8 relative z-10" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
        ...
        <button onClick={() => { playClick(); setEditingBook(null); setImageBase64(""); setIsModalOpen(true); }} className="flex items-center gap-2 px-4 py-2 bg-white/5 hover:bg-white/10 rounded-xl text-white text-xs font-bold transition-colors border border-white/5">
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar -mr-2 pr-2 relative z-10 space-y-8">
          ...
          <AnimatePresence mode="popLayout">
            {currentBook ? (
                <motion.div layoutId={currentBook._id} initial={{opacity:0, y:20}} animate={{opacity:1, y:0}} className="w-full bg-[#121212] border border-purple-500/30 rounded-[32px] p-6 flex flex-col md:flex-row gap-6 relative overflow-hidden group/active shadow-[0_0_30px_rgba(168,85,247,0.1)]" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
                    ...
                    <div className="flex-1 flex flex-col relative z-10 min-w-0">
                        ...
                        <div className="space-y-2 mb-6">
                            ...
                            <div className="w-full h-3 bg-black/50 rounded-full overflow-hidden border border-white/5">
                                <motion.div className="h-full bg-gradient-to-r from-purple-600 to-pink-500 rounded-full shadow-[0_0_15px_rgba(168,85,247,0.5)]" initial={{width:0}} animate={{width:`${(currentBook.completedUnits/currentBook.totalUnits)*100}%`}} transition={{duration: 1}} />
                            </div>
                        </div>
                        <div className="flex flex-wrap gap-2 mt-auto">
                            <button onClick={() => handleUpdateProgress(currentBook._id, 1)} className="flex-1 py-3 bg-white/5 hover:bg-white/10 text-white font-bold rounded-xl text-[10px] transition-colors border border-white/5 flex items-center justify-center gap-1 group/btn"><Plus size={12} className="text-gray-500 group-hover/btn:text-white transition-colors"/> 1 {txt.lib_pg_short || "PG"}</button>
                            <button onClick={() => handleUpdateProgress(currentBook._id, 5)} className="flex-1 py-3 bg-white/5 hover:bg-white/10 text-white font-bold rounded-xl text-[10px] transition-colors border border-white/5 flex items-center justify-center gap-1 group/btn"><Plus size={12} className="text-gray-500 group-hover/btn:text-white transition-colors"/> 5 {txt.lib_pg_short || "PG"}</button>
                            <button onClick={() => handleFinishBook(currentBook._id)} className="flex-1 py-3 bg-green-500/10 hover:bg-green-500 hover:text-black text-green-500 border border-green-500/20 font-bold rounded-xl text-[10px] transition-all flex items-center justify-center gap-2"><CheckCircle size={14}/> {txt.finish || "DONE"}</button>
          ...
          <div className="pt-2">
              ...
              <div className="flex overflow-x-auto gap-4 pb-6 custom-scrollbar snap-x">
                  {otherBooks.map((book) => (
                      <motion.div layoutId={book._id} key={book._id} onClick={() => { playClick(); setSelectedBook(book); }} onMouseEnter={() => playHover()} className="relative min-w-[140px] w-[140px] h-[220px] bg-[#121212] rounded-2xl border border-white/5 cursor-pointer overflow-hidden group shadow-lg flex-shrink-0 snap-start hover:translate-y-[-5px] transition-all duration-300">
                          {book.image ? (
                  ...
                  <button onClick={() => { playClick(); setEditingBook(null); setImageBase64(""); setIsModalOpen(true); }} className="min-w-[140px] w-[140px] h-[220px] border-2 border-dashed border-white/10 rounded-2xl flex flex-col items-center justify-center text-gray-600 hover:text-purple-400 hover:border-purple-500/30 transition-all gap-3 flex-shrink-0 bg-white/[0.02] hover:bg-white/[0.05] group">
