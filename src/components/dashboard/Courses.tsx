...
export default function Courses({ courses: initialCourses }: { courses: any[] }) {
  ...
  const otherCourses = courses.filter(c => c.status === 'idle'); // ✅ هذا يضمن عدم ظهور الـ completed
  ...
  const handleStart = async (id: string) => { 
      ...
      // ✅ إصلاح المشكلة هنا:
      // نغير حالة الكورس المختار فقط، ونعيد الكورس النشط سابقاً لقائمة الانتظار
      // ونترك الكورسات المنتهية (completed) كما هي
      const updated = courses.map(c => {
          if (c._id === id) return { ...c, status: 'in_progress' }; // الكورس الجديد يصبح نشط
          if (c.status === 'in_progress') return { ...c, status: 'idle' }; // الكورس النشط سابقاً يعود للقائمة
          return c; // الباقي (مثل completed) يبقى كما هو
      });
      
      setCourses(updated); 

      // ✅ 2. إظهار التوست
      toast.success(txt.toast_course_active || "Module Activated 🚀", {
        style: { background: "#0891b2", color: "#fff", border: "1px solid #0e7490" }
      });
  ...
  const handleCreate = async (e: React.FormEvent<HTMLFormElement>) => {
      ...
      if(res.success) { 
          ...
          // ✅ توست الإضافة
          toast.success(txt.toast_course_added || "Module Initialized Successfully 🎓");
  ...
  const handleDelete = async () => { 
      ...
      // ✅ توست الحذف
      toast(txt.toast_course_deleted || "Module Deleted 🗑️", {
  ...
  const handleFinish = async (e: React.FormEvent<HTMLFormElement>) => {
      ...
      if(res.success) { 
          ...
          // ✅ تحديث الحالة: نحول الكورس إلى completed حتى يختفي من الفلاتر
          const updated = courses.map(c => 
            c._id === certCourse._id ? { ...c, status: 'completed' } : c
          );
          setCourses(updated);

          // ✅ توست الإنهاء (ذهبي)
          toast.success(txt.toast_course_completed || "Certificate Earned! 🏆", {
  ...
  const Modals = () => {
      ...
      return createPortal(
          <AnimatePresence>
              {isAddOpen && (
                  <motion.div key="modal-add" className="fixed inset-0 z-[200] bg-black/80 backdrop-blur-xl flex items-center justify-center p-4" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}>
                      <motion.div 
              ...
              {certCourse && (
                  <motion.div key="modal-cert" className="fixed inset-0 z-[200] bg-black/90 backdrop-blur-xl flex items-center justify-center p-4" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}>
                      <motion.div 
              ...
              {deleteId && (
                  <motion.div key="modal-delete" className="fixed inset-0 z-[250] bg-black/90 backdrop-blur-md flex items-center justify-center p-4" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}>
                      <motion.div 
    ...
    <div className="bg-gradient-to-b from-[#0F0F0F] to-[#050505] rounded-[40px] border border-white/5 p-8 h-[750px] flex flex-col relative overflow-hidden group shadow-2xl" dir="ltr">
      ...
      <div className="relative z-10 shrink-0 mb-6">
          <AnimatePresence mode="popLayout">
              {activeCourse ? (
                  <motion.div layoutId="active-course" initial={{opacity:0, y:20}} animate={{opacity:1, y:0}} exit={{opacity:0, scale:0.95}} className="relative" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
